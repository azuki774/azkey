...
Cursor 利用時は `.cursor/rules/fork-and-release.md` に同じ内容があり、自動的に読み込まれる。
...
- upstream からの取り込み用に PR を作成する場合は、**PR 本文中に、どのファイルでどのようにコンフリクトを解消したかを明記する**（例: どちら側を優先したか、手動で統合した箇所など）。
- 併せて以下を更新する:
